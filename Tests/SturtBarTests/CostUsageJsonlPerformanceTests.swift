...
struct CostUsageJsonlPerformanceTests {
    ...
    func `scanner benchmark beats front buffer baseline`() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "sturtbar-cost-usage-bench-\(UUID().uuidString)",
            isDirectory: true)
        ...
        // Allow 15% headroom over the baseline to avoid CI flakes on loaded machines.
        #expect(currentFastest < UInt64(Double(baselineFastest) * 1.15))
    }
}
