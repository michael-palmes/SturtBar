...
- Re-authentication belongs to the CLI. The card's sign-in button writes a small helper script to `~/.sturtbar` and opens it in the user's default terminal; the terminal runs `claude /login` from that folder, so any Claude Code workspace prompt covers only SturtBar's own folder holding that one script. SturtBar itself never runs the claude CLI and never touches its credential stores.
