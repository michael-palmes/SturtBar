...
/// Finds Claude Desktop's embedded `.claude/projects` stores; opt-in via `includeClaudeDesktopRoots`.
enum ClaudeDesktopProjectsLocator {
    private static let sessionDirectoryNames = [
        "local-agent-mode-sessions",
        "claude-code-sessions",
    ]

    /// Heavy or irrelevant children found inside session directories; never descended into.
    ...
    /// Covers observed Desktop layouts without descending into arbitrary checked-out workspaces.
