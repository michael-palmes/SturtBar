...
/// Codable so Phase 3 can persist the last cost snapshot to disk (`StatePersistence`).
/// `Entry`/`ModelBreakdown` gained explicit `Encodable` conformances (canonical keys only) for the
/// same reason; decoding still accepts the legacy alternate keys.
...
// MARK: - Encodable (canonical keys)

/// `Entry`/`ModelBreakdown` decode from several legacy key spellings; encoding always writes the
/// canonical keys so persisted state round-trips through the same decoders.
...
extension CostUsageDailyReport {
    ...
    static func sortedModelBreakdowns(_ breakdowns: [ModelBreakdown]) -> [ModelBreakdown] {
