...
/// Ported as-is from CodexBar's CostUsageScanner+Claude with:
/// - models.dev catalog pricing (Phase 2b): the catalog is loaded once per
///   loadClaudeDaily call and threaded down through ClaudeScanState and into
///   buildClaudeReportFromCache to avoid per-row file reads,
/// - the bounded-scan mtime skip (see "Bounded scan" below).
extension CostUsageScanner {
    ...
    static func parseClaudeFile(
        ...
        startOffset: Int64 = 0,
        modelsDevCatalog: ModelsDevCatalog? = nil) -> ClaudeParseResult
    {
        (
            try? self.parseClaudeFileCancellable(
                ...
                modelsDevCatalog: modelsDevCatalog)) ?? ClaudeParseResult(rows: [], parsedBytes: startOffset)
    }
        ...
        modelsDevCatalog: ModelsDevCatalog? = nil) throws -> ClaudeParseResult
    {
        ...
        do {
            ...
            // On non-cancellation read error return no rows and reset parsedBytes
            // to startOffset so the next scan retries from the same position,
            // preventing double-counting of any partial rows that were accumulated.
            return ClaudeParseResult(rows: [], parsedBytes: startOffset)
        }
    ...
    private static func reconciledClaudeRows(
    ...
    private static func rebuildClaudeDays(
        cache: inout CostUsageCache,
        reconciledRows: [ClaudeUsageRow],
        checkCancellation: CancellationCheck? = nil) throws
    {
        var days: [String: [String: [Int]]] = [:]

        for (index, row) in reconciledRows.enumerated() {
            if index & 4095 == 0 { try checkCancellation?() }
    ...
    private final class ClaudeScanState {
        ...
        /// models.dev catalog loaded once for the scan; nil means no catalog available.
    ...
    // MARK: - Bounded scan (rebuild performance bound)

    /// Files last modified more than a day before the scan window start cannot
    /// contain rows inside the window: rows are appended as they happen, so a
    /// file's newest row timestamp is bounded by its modification time (with a
    /// one-day buffer for timezone skew, matching the range's scan buffer).
    /// Skipping them avoids opening and parsing old session logs entirely.
    private static func claudeMtimeSkipCutoffMs(range: CostUsageDayRange) -> Int64? {
        ...
        state: ClaudeScanState) throws
    {
        ...
        // Bounded scan (rebuild performance bound): see claudeMtimeSkipCutoffMs.
        let mtimeSkipCutoffMs = Self.claudeMtimeSkipCutoffMs(range: state.range)
        ...
        for case let url as URL in enumerator {
            ...
            // Bounded scan (rebuild performance bound): the file was last
            // modified before the window, so it cannot contain in-window rows.
            // Leaving it untouched also drops any stale cache entry for it.
        ...
        checkCancellation: CancellationCheck?) throws -> CostUsageDailyReport
    {
        ...
        // The Claude scan always excludes Vertex AI rows (vertex billing is not
        // part of the Anthropic transcript spend this scanner reports).
        let providerFilter = ClaudeLogProviderFilter.excludeVertexAI

        // Load the models.dev catalog once for the full scan / report pass.
        // Passing it explicitly avoids per-row disk reads inside the scan loop.
        ...
        if shouldRefresh {
            ...
            let reconciledRows = try Self.reconciledClaudeRows(cache: cache, checkCancellation: checkCancellation)
            try Self.rebuildClaudeDays(
                cache: &cache,
