...
enum ClaudeOAuthUsageFetcher {
    ...
    static func fetchUsage(
        accessToken: String,
        transport: any HTTPTransport = HTTPClient.shared) async throws -> OAuthUsageResponse
    {
        ...
        // SturtBar never spawns the Claude CLI to detect a version; always send the pinned constant.
        request.setValue("claude-code/\(Self.fallbackClaudeCodeVersion)", forHTTPHeaderField: "User-Agent")
        ...
        do {
            ...
            case 429:
                ...
                // Use the gate's resolved block date so the throw is consistent with what the gate
                // will return on subsequent calls. Fall back to the header value or a 5-minute default.
                let resolvedRetryAfter = ClaudeOAuthUsageRateLimitGate.currentBlockedUntil(now: now)
                    ?? retryAfter
                    ?? now.addingTimeInterval(300)
                throw ClaudeOAuthFetchError.rateLimited(retryAfter: resolvedRetryAfter)
            case 403:
