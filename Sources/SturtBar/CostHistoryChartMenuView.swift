// CostHistoryChartMenuView.swift — Swift Charts daily-cost bar chart for the cost submenu.
//
// Ported from legacy CodexBar/CostHistoryChartMenuView.swift (Phase 4a). Rebuild changes:
//   - Provider parameter dropped; bars use the Claude brand color.
//   - `L(...)` localization → literals.
//   - Convenience init straight from `CostUsageTokenSnapshot` (daily + totals + window label).
// Phase 4b hosts this in a submenu item. The hover detail block already practices fixed-height
// discipline (legacy behavior): it reserves the tallest breakdown layout across all days so
// hovering different bars never resizes the menu view.
...
struct CostHistoryChartMenuView: View {
    ...
    init(snapshot: CostUsageTokenSnapshot, width: CGFloat) {
        self.init(
    ...
    var body: some View {
        let model = Self.makeModel(daily: self.daily)
        VStack(alignment: .leading, spacing: 10) {
            if model.points.isEmpty {
                Text("No cost history data.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                    .accessibilityLabel("No cost history data.")
            } else {
                Chart {
                ...
                .accessibilityValue(
                    model.points.isEmpty
                        ? "No data"
                        : "\(model.points.count) days of cost data")
    ...
    #if DEBUG
    private static let hoverLog = SturtBarLog.logger("chart-hover")
    #endif
    // 4b: verify under accessibility text sizes; heights are pt-fixed and may clip under larger
    //     Dynamic Type scales. Replace with NSFont-derived line heights when the detail block is
    //     reworked (caption/caption2 line heights at default size are ≈16pt / ≈13pt respectively).
    ...
    private static func makeModel(daily: [DailyEntry]) -> Model {
        ...
        geo: GeometryProxy)
    {
        ...
        if self.selectedDateKey != nearest {
            ...
            #if DEBUG
            // Live-verification probe (Phase 4b): hover selection inside the real NSMenu
            // submenu window is only observable via logs (screenshots are TCC-gated).
            Self.hoverLog.debug("Chart hover selection changed", metadata: ["dateKey": "\(nearest)"])
            #endif
        }
    }
...
#Preview("Cost history chart") {
    ...
    let daily: [CostUsageDailyReport.Entry] = [
        .init(
            ...
            modelBreakdowns: [
                .init(
                    ...
                    costUSD: 1.20,
                    totalTokens: 600_000,
                    standardCostUSD: 0.90,
                    priorityCostUSD: 0.30),
                .init(modelName: "claude-sonnet-4-5", costUSD: 0.54, totalTokens: 290_000),
            ]),
        .init(
            date: dayKey(5),
            inputTokens: 0,
        ...
        .init(
            ...
            modelBreakdowns: [.init(modelName: "claude-opus-4-6", costUSD: 3.51, totalTokens: 1_340_000)]),
        .init(
            date: dayKey(3),
            inputTokens: 500_000,
            outputTokens: 60000,
            ...
            modelBreakdowns: [
                .init(modelName: "claude-sonnet-4-5", costUSD: 0.92, totalTokens: 560_000),
            ]),
        .init(
            date: dayKey(2),
            ...
            costUSD: 5.88,
            modelsUsed: ["claude-opus-4-6", "claude-sonnet-4-5", "claude-haiku-3-5"],
            modelBreakdowns: [
                .init(
                    modelName: "claude-opus-4-6",
                    costUSD: 4.10,
                    totalTokens: 1_200_000,
                    standardCostUSD: 3.00,
                    priorityCostUSD: 1.10),
                .init(modelName: "claude-sonnet-4-5", costUSD: 1.20, totalTokens: 650_000),
                .init(modelName: "claude-haiku-3-5", costUSD: 0.58, totalTokens: 160_000),
            ]),
        .init(
            date: dayKey(1),
            inputTokens: 400_000,
            ...
            costUSD: 0.68,
            modelsUsed: ["claude-sonnet-4-5"],
            modelBreakdowns: [.init(modelName: "claude-sonnet-4-5", costUSD: 0.68, totalTokens: 450_000)]),
        .init(
            date: dayKey(0),
            ...
            modelsUsed: ["claude-opus-4-6", "claude-sonnet-4-5"],
            modelBreakdowns: [
                .init(modelName: "claude-opus-4-6", costUSD: 1.80, totalTokens: 750_000),
                .init(modelName: "claude-sonnet-4-5", costUSD: 0.65, totalTokens: 370_000),
            ]),
    ]
    return CostHistoryChartMenuView(
        daily: daily,
        totalCostUSD: 15.18,
        historyDays: 7,
