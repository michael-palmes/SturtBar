// UsageProgressBar.swift — static progress fill used inside the menu card.
//
// Ported as-is from legacy CodexBar/UsageProgressBar.swift (Phase 4a) — single-Canvas drawing
// (track + fill + pace punch-out + quota markers) with no SwiftUI compositing modifiers, which
// avoids the Metal/RenderBox shader compilation that broke the status item on macOS 26.x
// (legacy issue #805).
//
// Fixed-height note: the bar is a constant 6pt tall regardless of data (markers and the pace tip
// draw inside that height); rows that contain it never change height when pace/markers appear.
...
struct UsageProgressBar: View {
    ...
    private static func paceStripeWidth(for scale: CGFloat) -> CGFloat {
        // 2pt snapped to the nearest pixel at the display scale (e.g. 1px on @1x, 4px on @2x).
        let pixels = (2 * max(scale, 1)).rounded()
        return pixels / max(scale, 1)
    }
    ...
    /// Overrides the default "\(n) percent" accessibility value. Pass "no data" for placeholder
    /// bars so VoiceOver announces something meaningful instead of "0 percent".
    ...
    var body: some View {
        ...
        .accessibilityValue(self.accessibilityValueOverride ?? "\(Int(self.clamped)) percent")
    }
