// MouseLocationReader.swift — NSView-based mouse tracking with local coordinates.
//
// Ported as-is from legacy CodexBar/MouseLocationReader.swift (Phase 4a). Used by the cost
// history chart's hover-to-inspect overlay; SwiftUI's `onHover` has no location, and NSMenu
// popups are not key windows so `.activeAlways` tracking is required.
